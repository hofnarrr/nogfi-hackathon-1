...
def change_inteface_name(interface_name: str ="Ethernet0", description: str = "reserved"):
    with httpx.Client(verify=False, auth=auth) as client:
        ...
        try:
            logger.info(f"Current desc on interface {interface_name}: {response.json()['openconfig-interfaces:description']}")
        except KeyError:
